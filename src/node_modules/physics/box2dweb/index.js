...
listener.BeginContact = function(contact) {
	...
	if(fist.isWeapon && fist.isAttacking && person.isPerson) {
		impulseVec.SetV(bodyPerson.GetPosition());
		impulseVec.Subtract(bodyFist.GetPosition());
		impulseVec.Normalize();
		impulseVec.Multiply(fist.pushBack);
		bodyPerson.ApplyImpulse(impulseVec, bodyFist.GetPosition());
		person.applyDamage(fist.getDamage() * person.strength);
	}
...
function queueDespawnBody(body) {
	bodiesToDespawn.push(body);
}
...
function despawnMesh(mesh) {
	if(mesh.attachMeshToUponDespawn) {
		// THREE.SceneUtils.detach(mesh, mesh.parent, world.scene);
...
function getClosestHeroBody(from, cutoffDistance) {
	var closest;
