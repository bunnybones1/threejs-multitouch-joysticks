...
function getClosestEnemyBody(from, cutoffDistance) {
	...
	var pos;
	enemyBodies.forEach(function(body){
		temp.SetV(from);
...
function spawnEnemy() {
	...
	circleParams.x = Math.random() * 6000 - 3000;
	circleParams.y = Math.random() * 500 + circleParams.radius;
	var body = addActor(circleParams);
	body.m_linearVelocity.x = (Math.random() - 0.5) * 3;
	body.m_linearVelocity.y = (Math.random() - 0.5) * 3;
...
function init() {
	...
	initd = true;
	scene = world.scene;
	floorBody = addActor(wallParams);
	wallParams.y = 600;
	ceilingBody = addActor(wallParams);
