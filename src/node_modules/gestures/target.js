...
var callback = function(x, y, vx, vy) {
	if(Math.sqrt(vx*vx+vy*vy) > 0.1) {
		console.log('DODGE!', vx, vy);
	} else {
		console.log('TARGET!', vx, vy);
		x = x * 2 - 1;
		y = y * 2 - 1;
		var pos1 = new THREE.Vector3(x, y, 0.5);
		var pos2 = new THREE.Vector3(x, y, 1);
		pos1.unproject(world.camera);
		pos2.unproject(world.camera);
		var rayVec = pos1.clone().sub(pos2);
		rayVec.multiplyScalar(5);
		var ray = new THREE.Ray(pos1, rayVec);
		// var yeah = ray.isIntersectionPlane(plane);
		var hit = ray.intersectPlane(plane);
		if(hit) {
			var avatar = physics.getAvatar();
			var desiredLocation = avatar.desiredLocation;
			desiredLocation.x = hit.x * physics.SCALE;
			desiredLocation.y = hit.y * physics.SCALE;
			avatar.target = physics.getClosestEnemyBody(desiredLocation, 0.25);
			world.targetSpriteAt(avatar.target || desiredLocation);
		}
	}
};
...
function onEnd(touches) {
	...
	callback(
		...
		mesh.originY,
		mesh.x - mesh.originX, 
		(mesh.y - mesh.originY) * view.aspectRatio
	);
}
