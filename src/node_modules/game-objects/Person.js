...
function Person(params) {
	...
	this.healingSpeed = params.healingSpeed;
	...
	this.arms = [arm, arm2];
	this.height = c + 20;
	this.radius = this.fixture.m_shape.m_radius;
	// this.addBox(30, 30, 30, 0, 0, 80, colors.tan, colors.red);
	if(!this.braindead) {
		setInterval(this.think.bind(this), params.interval);
	}
}
...
Person.prototype.getHealthPercent = function() {
...
Person.prototype.death = function() {
	...
	var parts = this.mesh.children.slice();
	var mesh = this.mesh;
	parts.forEach(function(partMesh){
		partMesh.position.z -= halfHeight;
	});
	mesh.rotateX(Math.PI * 0.5);
	physics.queueDespawnBody(this.body);
	var deathValue = {value: 0};
	var fam = false;
	var _this = this;
	tweener.to(deathValue, 2, {
		value: 3,
		onUpdate: function() {
			if(deathValue.value > 1) {
				mesh.visible = ~~((deathValue.value * 10) % 2) === 0;
			}
			if(deathValue.value >=2 && !fam) {
				world.death(_this);
				fam = true;
			}
		},
	...
	scenery.registerTeleportee(this.mesh);
	console.log('Dead!');
};

Person.prototype.getClosestInterestingThing = function() {
	return null;
};

Person.prototype.think = function()  {
	if (Math.random() > 0.5) {
		this.target = this.getClosestInterestingThing();
		if (!this.target) {
...
Person.prototype.applyDamage = function(damage) {
...
Person.prototype.updateWalk = function() {
	...
	this.health = clamp(this.health + this.healingSpeed, 0, this.maxHealth);

	var body = this.body;
	var walkTo;
	var acceptableDistance = 0;
	var currLocation = body.GetPosition();

	if(this.target){
		if(!this.target.GetUserData().alive) {
			this.target = null;
		} else {
			acceptableDistance = this.target.m_userData.radius + body.m_userData.radius + 0.04;
			walkTo = this.target.GetPosition();
		}
	}
	if(!this.target){
		acceptableDistance = 0.05;
	...
	body.ApplyForce(this.walkForce, body.GetPosition());
	angleDelta = clamp(angleDelta, -0.1, 0.1);
	body.ApplyTorque(angleDelta * 0.04);
};
