...
function Person(params) {
	...
	var c = 0;
	//pants
	this.addBox(65, 50, 1, 0, 0, 0, 'darkblue', undefined, 0.3);
	...
	this.addBox(thickness, length, thickness, 25, length*0.5-thickness, c+h*0.5, colorSkin, colorSkin2);
	this.addBox(thickness, length, thickness, -25, length*0.5-thickness, c+h*0.5, colorSkin, colorSkin2);
	c+=h*0.5+5;
	...
	c+=h*0.5;
	//face
	this.addBox(6, 6, 6, 0, 20, c, colorSkin, colorSkin2);
	...
	if(Math.random() < 0.8) {
		...
		this.addBox(35, 20, 20, 0, -10, c+10, colorHair, colorHair);
		if(Math.random() < 0.75) {
			hatHeight += 10;
			hatBack -=5;
			this.addBox(35, 35, 15, 0, 0, c+25, colorHair, colorHair);
		}
	}
	//hat
	if(Math.random() < 0.15) {
...
Person.prototype.getGeometry = function (radius) {
	if (!geometry){
		geometry = new THREE.SphereGeometry(radius*0.1, 6, 4)
	}
	return  geometry
}
