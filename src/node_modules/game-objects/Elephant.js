...
function Elephant(params) {
	...
	var elephantMesh = new THREE.Mesh(new THREE.SphereGeometry(params.radius, 16, 8));
	var towerLayers = 3;
	var towerFloorHeight = 40;
	var towerFloorOffset = params.radius * 1.05;
	var floorGeometry = new THREE.BoxGeometry(70, towerFloorHeight, 20);
	for (var i = 0; i < towerLayers; i++) {
		var floorMesh = new THREE.Mesh(floorGeometry);
		floorMesh.position.y = towerFloorOffset + i * towerFloorHeight;
		elephantMesh.add(floorMesh);
	}
	params.mesh = elephantMesh;
	...
	this.archerTowerLayerHeight = 0.09;
...
Elephant.prototype.fireArrow = function(vx, vy) {
	...
	var layer = ~~(Math.random() * 3);
	var arrow = new Arrow({
		x: position.x * physics.SCALE_INV,
		y: (position.y + this.archerOffset + layer * this.archerTowerLayerHeight) * physics.SCALE_INV,
