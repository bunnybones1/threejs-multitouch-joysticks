...
Elephant.prototype.castSightRay = function(x, y, vecX, vecY,index) {
	...
	if(this.nearestPoint) {
		this.getSightMesh(index).position.set(this.nearestPoint.x * physics.SCALE_INV, this.nearestPoint.y * physics.SCALE_INV, 0);
	} else {
		this.getSightMesh(index).position.set((x+vecX) * physics.SCALE_INV, (y+vecY) * physics.SCALE_INV, 0);
	}
};

Elephant.prototype.getSightMesh = function(index) {
	if(!this.sightMeshes[index]){
		this.sightMeshes[index] = new three.Mesh(new three.SphereGeometry(20, 16, 8));
