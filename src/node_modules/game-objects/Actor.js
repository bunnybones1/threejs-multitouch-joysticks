...
var boxMaterials = {};
function getBoxMaterial(color, ambientColor) {
	if(!boxMaterials[String(color)]) {
		boxMaterials[String(color)] = new THREE.MeshLambertMaterial({
			color: color,
			emissive: ambientColor
		});
	}
	return boxMaterials[String(color)];
}

Actor.prototype.addBox = function(w, d, h, x, y, z, color, ambientColor){
	var box = new THREE.Mesh(getBoxGeometry(), getBoxMaterial(color||0xffffff, ambientColor));
	this.mesh.add(box);
